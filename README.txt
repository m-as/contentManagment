...
This JavaScript library contains a set of functions which can be used for adding all sorts of contents to your website.
The code has been written in pure JavaScript without the use of any JavaScript Library like jQuery, therefore it can be used in all the pages in our system.
...
tag:
HTML tag type of the element we’re targeting with a class. This parameter has to be set in case different HTML tags are using the same class. If passed blank all elements with the same class will be targeted.
...
newID:
For all functions were we can pass criteria’s, this parameter defines which Query String parameter in the URL we’re targeting.
